...
p4h_from = "!$%&',;<=>?\^`|~/{}ABCDEFGHIJKLMNOPQRSTUVWXYZ01234567890#()[]*+-_.:"
p4h_to   = "!$%&',;<=>?\^`|~/{}TQR0HBPCKG9ALY80NIMJ42DE1XWUS65Z7O3FV#()[]*+-_.:"
p4h_parts = str.maketrans(p4h_from,p4h_to)
...
class SyncDocumentType(models.Model):
    ...
    def _import_products(self, conn, sync_action_id, mif_id):
        ...
        if not self.skip_import:
            ...
            self.log_note += 'Imported %s parts' % (parts)
            self.state = 'done'
        ...
        for model in models:
            ...
            if not model_id:
                ...
                vals = {
                    ...
                    'p4h_code': 'P-' + model_code.translate(p4h_parts),
                    'mif_id': mif_id,
    ...
    def _extract_parts_datas(self, conn, row, workbook, worksheet, heater_codes, used_in_row, category_ids, application_fields, fields_index, index_categories_ids, attribute_data, mif_id):
        ...
        for r in range(row, worksheet.nrows):
            ...
            res = dict(
                ...
                p4h_code='P-'+part_no.translate(p4h_parts),
                name=product_name,
                mif_id=mif_id,
                # image=self.product_img_base64,
                type='product',
